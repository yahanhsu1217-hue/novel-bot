...
def _world_str(world_mode: str, world_input: str) -> str:
    ...
    return (
        f"【世界背景設定 — 只是背景知識，不是劇情觸發點】\n"
        f"{world_input}\n"
        f"【重要】以上是這個世界存在的背景規則與環境描述，不是即將發生在主角身上的劇情安排。"
        f"其中提到的任何機制、模式、事件，除非用戶在「希望出現的劇情元素」或「本章特別指示」中明確要求，"
        f"否則不得主動觸發或套用到主角及劇情上。"
    )
    ...
    for c in (cp_characters or []):
        ...
        if notes:
            guards.append(
                ...
                f"{notes}\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"【⛔ 第 {chapter_num} 章硬性禁令 — 無條件執行，任何理由都不能解除】\n\n"
                f"⛔ 禁令一（最高級別，全故事適用直到備注明確解禁）：\n"
                f"   {name} 本章絕對不可告白。\n"
                f"   禁止說出：「我喜歡你」「我愛你」「你對我來說不只是朋友」「我對你有感覺」\n"
                f"   或任何語義等同於向主角表達愛慕的句子。\n"
                f"   就算劇情氣氛再好、再浪漫，本章告白就是失敗。\n\n"
                f"⛔ 禁令二（依備注判斷本章階段後執行）：\n"
                f"   閱讀以上備注，確認第 {chapter_num} 章屬於哪個情感階段，然後：\n"
                f"   - 若屬於「無戀愛感受」階段：禁止心跳加速、臉紅、深情凝視、主動靠近、暗示喜歡\n"
                f"   - 若屬於「哪裡不對勁」階段：只能一閃而過的不適感，事後她必須自動找理由解釋掉\n"
                f"   - 若屬於「開始動搖」階段：可以困惑，但仍然排斥，不可接受，絕不告白\n\n"
                f"⛔ 禁令三（主角行為限制）：\n"
                f"   主角（我）本章同樣不可向 {name} 告白、表白、說出喜歡/愛。\n"
                f"   {name} 的感情尚未發展到能接受告白的程度；此時主角告白只會讓 {name} 陷入必須回應的困境，\n"
                f"   破壞整條感情線的節奏。主角告白視同整章失敗。\n\n"
                f"⛔ 禁令四（強迫情境漏洞封堵 — 本故事如有任務/系統強迫親密接觸，此禁令必須執行）：\n"
                f"   強迫性情境（如系統任務、情境壓力）不能作為 {name} 提前產生浪漫感情的理由。\n"
                f"   她可以有生理反應（緊張、心跳快），但這些反應必須被她本人解讀為「情況讓我緊張」，\n"
                f"   絕對禁止她思考「不知道是情況還是你讓我這樣」「也許不只是任務」等開始懷疑自己感情的念頭。\n"
                f"   以下句型在本章嚴格禁止，即使換詞也不行：\n"
                f"   ✗「我不知道是因為任務，還是因為你」\n"
                f"   ✗「我不想讓這件事變得只是任務」\n"
                f"   ✗「如果我們想，而不是因為系統逼我們」（暗示她對主角有主動意願）\n"
                f"   ✗ 任何表示她對親密互動有超出任務需求的情感依戀的言行\n\n"
                f"✅ 強制執行：{name} 在本章對強迫親密接觸的正確反應模板（必須選一種或混合使用）：\n"
                f"   【煩躁型】她在執行任務時帶著明顯的不耐煩或挑剔，完成後第一件事是吐槽或找碴，\n"
                f"   不是沉默地感受餘韻。例：任務完成後她立刻抱怨規則荒謬，或調侃主角表現，不會停在那個氛圍裡。\n"
                f"   【尷尬型】她明顯不自在，會用行動或話題打破氣氛，主動製造距離，\n"
                f"   而不是留在親密空間裡讓氣氛繼續發酵。例：站起來走開、低頭找事情做、突然問一個無關的問題。\n"
                f"   【不情願型】她接受任務但全程帶著「這很荒謬」「我不想但沒辦法」的心態，\n"
                f"   事後的第一個念頭是「終於結束了」，而不是「好像不太糟」。\n"
                f"   【冷靜執行型】她把任務當成純粹的生存需求，像做伏地挺身一樣，\n"
                f"   全程沒有情緒，完成後直接切換到下一個話題，就好像什麼都沒發生。\n"
                f"   ⚠️ 禁止混用：不可以在「不情願執行」之後緊接著「柔軟的餘韻」，這樣的轉折本身就是在暗示她有感覺。\n\n"
                f"⛔ 本章感情上限（根據備注明確設定，不可超越）：\n"
                f"   第 {chapter_num} 章允許的最高感情程度，由以上備注決定。\n"
                f"   超越備注所描述的情感程度，即視為失敗。\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            )
    if not guards:
        return ""
