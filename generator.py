...
【完整性愛場景的必要結構 — 每個場景都必須包含以下五個階段，每階段最少 150 字】
① 前戲鋪陳：衣物、位置、肢體接觸的具體過程，雙方的生理初反應（呼吸、皮膚、器官狀態）
② 升溫推進：主要動作開始，力道、節奏、角度隨時間變化，生理反應逐步強烈（分泌、勃起、漲大、潮紅）
③ 對話與心理：場景中穿插角色的真實對話（情慾狀態下的語言）與內心反應（慾望、掙扎、沉淪、享受）
④ 高潮呈現：以完整段落描寫高潮的生理過程，包含顫抖方式、聲音、液體、身體失控的細節
⑤ 餘韻：事後的身體狀態（液體、氣息、疲憊）、環境細節、角色的情緒或沉默

缺少任何一個階段，或任何一個階段不足 150 字，即視為場景不完整，必須補足。
