...
- 【每句話必須推進場景，不可重播狀態】寫每一句前先問：「這句話描述的，是這個瞬間新發生的變化，還是在重複已經存在的狀態？」若是重複狀態，這句話必須刪除或改寫成新的變化；場景中的每個句子都必須讓某件事改變——位置、力道、節奏、情緒、身體反應、意識狀態、權力關係——至少其中一項；若刪掉這句話，場景意義完全不變，那麼它不應該存在
