...
每個情節點必須包含：
① **粗體標題**（5-10字，概括該情節的核心動作或事件）
② 詳細描述（100字以上）：具體發生了什麼、誰做了什麼、說了什麼、對方如何反應、情緒如何變化
③ 不可只寫「XXX做了YYY」一句帶過，必須呈現事件的過程與張力

例：
1. **XXX的標題**：（詳細描述，100字以上，包含動作、對話方向、情緒反應、場景細節）
2. **XXX的標題**：（同上）
3. **XXX的標題**：（同上）
（根據指示步驟數量繼續列出，至少8-12個情節點，不必限制在5點）

---

**角色互動重點**
按角色配對逐一分析，每對至少3句：
- **[角色A] × [角色B]**：（描述這對之間的具體互動細節、眼神、動作、潛台詞）
- **[角色C] × [主角]**：（同上）
（以此類推，涵蓋本章所有重要互動配對）
...
**感情線進展**
按每個CP角色分別分析：
- **[角色名]**：（描述本章這個角色對主角情感的具體變化——從什麼狀態變成什麼狀態，是什麼事件促成，現在的心理位置在哪裡；必須符合感情備注的限制）
（以此類推）
...
**本章結尾**
{_ending_format_line}
（結尾必須是一段有畫面感的場景描寫，150字以上，包含具體的人物位置、動作、情緒狀態，以及留下的伏筆或懸念）

⚠️【強制規定】必須完整輸出以上所有段落，每個情節點不可流於表面、不可用「XXX感到緊張」代替具體事件描寫，本章結尾不可省略或截斷。

輸出語言：{language}
直接輸出大綱，不要前言或說明。"""

    _outline_sys_parts = [
        "你是一位連載小說的故事策劃，擅長規劃環環相扣、富有細節的章節走向。",
        "大綱的品質標準：讀完大綱，即使不看正文，也能清楚知道這章每一幕發生了什麼、角色說了什麼方向的話、情緒如何流動。",
        "每個情節點必須有：具體事件（發生什麼）+ 角色反應（誰怎麼做/說）+ 情緒轉折（心理變化）。缺一不可。",
        "禁止空洞描述：「兩人之間的氣氛變得微妙」「她感到一絲不安」這類不帶具體事件的句子，必須替換成實際發生的動作、對話方向或場景細節。",
